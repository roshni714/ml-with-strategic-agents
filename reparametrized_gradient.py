...
def plot_total_derivative(agent_dist, sigma, q, f, true_beta=None, savefig=None):
    deriv = []
    deriv_emp = []
    dx = 0.001
    ...
    for theta in thetas:
        ...
        hat_val = empirical_total_derivative(agent_dist, theta, s, sigma, q, true_beta)
        deriv.append(val)
...
def empirical_gradient_loss_s(agent_dist, theta, s, sigma, q, true_beta=None, perturbation_size=0.05):
    ...
    perturbed_noisy_scores =  np.clip(noisy_scores - perturbations, a_min=bounds[0], a_max=bounds[1])
...
def expected_total_derivative(agent_dist, theta, s, sigma, true_beta=None):
    ...
    gamma_loss_theta = expected_gradient_loss_theta(agent_dist, theta, s, sigma, true_beta)
...
def empirical_total_derivative(agent_dist, theta, s, sigma, q, true_beta=None):
    gamma_loss_s = empirical_gradient_loss_s(agent_dist, theta, s, sigma, q, true_beta)
    gamma_loss_theta = empirical_gradient_loss_theta(agent_dist, theta, s, sigma, q, true_beta)
    gamma_s_theta = empirical_gradient_s_theta(agent_dist, theta, s, sigma) 
    total_derivative = gamma_loss_s * gamma_s_theta + gamma_loss_theta
    return total_derivative
