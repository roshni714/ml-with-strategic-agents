...
def plot_grad_pi_beta(agent_dist, sigma, q, f, savefig=None):
    thetas = np.linspace(-np.pi, np.pi, 50)
    grad_betas_0 = []
    grad_betas_1 = []
    emp_grad_betas_0 = []
    emp_grad_betas_1 = []
    for theta in thetas:
        grad_beta = expected_gradient_pi_beta(agent_dist, theta, sigma, f)
        grad_betas_0.append(grad_beta[0])
        grad_betas_1.append(grad_beta[1])

        beta = np.array([np.cos(theta), np.sin(theta)]).reshape(2, 1)
        emp_grad_beta = empirical_gradient_pi_beta(agent_dist, beta, f(theta), sigma, q)
        print(grad_beta, emp_grad_beta)
        emp_grad_betas_0.append(emp_grad_beta[0])
        emp_grad_betas_1.append(emp_grad_beta[1])
    
    fig, ax = plt.subplots(1, 2, figsize=(12,5))
    ax[0].plot(thetas, emp_grad_betas_0, label="empirical")
    ax[0].plot(thetas, grad_betas_0, label="expected")
    ax[0].set_xlabel("Theta (Corresponds to Beta)")
    ax[0].set_ylabel("dpi/dbeta1")
    ax[0].set_title("Beta vs. dpi/dbeta1")
    ax[1].plot(thetas, emp_grad_betas_1, label="empirical")
    ax[1].plot(thetas, grad_betas_1, label="expected")
    ax[1].set_xlabel("Theta (Corresponds to Beta)")
    ax[1].set_ylabel("dpi/dbeta2")
    ax[1].set_title("Beta vs. dpi/dbeta2")
    plt.legend()
    if savefig is not None:
        plt.savefig(savefig)
    plt.show()
...
def expected_gradient_pi_beta(agent_dist, theta, sigma, f):
    dim = agent_dist.d
    assert dim==2, "Method does not work for dimension {}".format(dim)
...
def empirical_gradient_pi_beta(agent_dist, beta, s, sigma, q, true_beta=None, perturbation_size=0.1):
    ...
    non_treatments = noisy_scores <= np.quantile(noisy_scores, q)
...
def expected_gradient_loss_s(agent_dist, theta, sigma, f, true_beta=None):
    ...
    z = s - np.array([np.matmul(beta.T, x) for x in  br_dist]).reshape(len(br_dist), 1)
    prob = norm.pdf(z, loc=0., scale=sigma)
    vec = np.array([1 - np.matmul(beta.T, grad_s_dist[i]) for i in range(len(br_dist))]).reshape(agent_dist.n_types, 1)
    res = prob * vec * true_scores * agent_dist.prop.reshape(agent_dist.n_types, 1)

    d_l_d_s = np.sum(res)
    return d_l_d_s.item()

def empirical_gradient_loss_s(agent_dist, beta, s, sigma, q, true_beta=None, perturbation_size=0.05):
...
def plot_grad_loss_s(agent_dist, sigma, q, f, true_beta=None, savefig=None):
    ...
    thetas = np.linspace(-np.pi, np.pi, 50)
    for theta in thetas:
        d_l_d_s = expected_gradient_loss_s(agent_dist, theta, sigma, f, true_beta)
        beta = np.array([np.cos(theta), np.sin(theta)]).reshape(2, 1)
        emp_grad = empirical_gradient_loss_s(agent_dist, beta, f(theta), sigma, q, true_beta)
        emp_grad_s.append(emp_grad)
