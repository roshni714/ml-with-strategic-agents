...
class GradientEstimator:
    def __init__(
        ...
        self.perturbations_s = (
            2
            * bernoulli.rvs(p=0.5, size=int(agent_dist.n / 2)).reshape(
                int(agent_dist.n / 2), 1
            )
            - 1
        ) * perturbation_s_size
    ...
    def get_theta_perturbed_scores(self):
        ...
        for agent_type in range(self.agent_dist.n_types):
            for p_theta in p_thetas:
                ...
                best_responses[agent_type].append({"p_theta": p_theta, "br": br})
        ...
        noisy_unperturbed_scores = (
            np.array(unperturbed_scores).reshape(int(self.agent_dist.n / 2), 1)
            + self.noise[: int(self.agent_dist.n / 2)]
        )
    ...
    def get_s_perturbed_scores(self):
        ...
        for agent_type in range(self.agent_dist.n_types):
            for p_s in p_ss:
                ...
                best_responses[agent_type].append({"p_s": p_s, "br": br})
    ...
    def compute_gradients(self, scores_s_perturbed, scores_theta_perturbed, cutoff):
        ...
        indicators_s = (
            scores_s_perturbed + self.perturbations_s
        ) < cutoff  # remove perturbations for estimating CDF
        ...
        loss_vector = np.concatenate((loss_vector_s, loss_vector_theta)).reshape(
            self.agent_dist.n, 1
        )
    ...
    def compute_total_derivative(self):
        ...
        (
            scores_theta_perturbed,
            scores_theta_unperturbed,
        ) = self.get_theta_perturbed_scores()
        scores_s_perturbed, scores_s_unperturbed = self.get_s_perturbed_scores()
        scores = np.concatenate(
            (scores_s_unperturbed, scores_theta_unperturbed)
